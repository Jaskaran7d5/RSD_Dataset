...
T = 0.5;                % Duration (s) -- increased for more samples
dt = 1/Fs;              % Time step
...
% === Pursuers (Echelon Right Formation) ===
pursuers = {
    % Leader (500m behind target, centered)
    struct('offset', [-500; 0; 0], 'speed_factor', 0.8),
    % #2: 800m behind, 400m right of leader
    struct('offset', [-800; 400; 0], 'speed_factor', 0.8),
    % #3: 1100m behind, 800m right of leader
    struct('offset', [-1100; 800; 0], 'speed_factor', 0.8)
};
...
save('vic_formation_pdw_dataset.mat', 'scenario', '-v7.3');
disp('Vic formation simulation complete. Dataset saved.');
